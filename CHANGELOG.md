...
- Deprecate `icupy.icu.Transliterator.register_instance()` and `.unregister()` (#220)
...
- Delete `icupy.icu.Transliterator.Token` class (#220)

### Fixed

- Fix `UTransPosition` changes in `icupy.icu.Transliterator._handle_transliterate()` to propagate to the caller (#220)
