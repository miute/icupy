...
from icupy.icu import (
    ErrorCode, Formattable, Locale, MeasureUnit, UErrorCode, UnicodeString,
    UnicodeStringAppendable, UNumberRangeCollapse,
    UNumberRangeIdentityFallback, UNumberRangeIdentityResult,
)
...
def test_formatted_number_range_63():
    fmt = (
        ...
        .number_formatter_first(
            NumberFormatter.with_().unit(MeasureUnit.create_meter())
        )
        .number_formatter_second(
            NumberFormatter.with_().unit(MeasureUnit.create_kilometer())
        )
    )
...
def test_formatted_number_range_69():
    ...
    assert cfpos.get_category() == UFieldCategory.UFIELD_CATEGORY_NUMBER
    assert (
        cfpos.get_field() == UNumberFormatFields.UNUM_GROUPING_SEPARATOR_FIELD
    )
...
def test_localized_number_range_formatter_64():
    ...
    assert isinstance(fmt, LocalizedNumberRangeFormatter)
    assert (
        fmt.format_formattable_range(
            Formattable(1), Formattable(5)
        ).to_string()
    ) == "1 m \u2013 5 m"  # 1 m - 5 m
