...
from icupy.icu import (
    ...
    u_get_unicode_version, u_has_binary_property, u_is_id_ignorable,
    u_is_id_part, u_is_id_start, u_is_iso_control, u_is_java_id_part,
    u_is_java_id_start, u_is_java_space_char, u_is_mirrored, u_is_ualphabetic,
...
def test_api():
    ...
    assert (
        u_char_from_name(UCharNameChoice.U_UNICODE_CHAR_NAME, "Cherry Blossom")
        == 0x1F338
    )
    assert (
        u_char_from_name(
            UCharNameChoice.U_EXTENDED_CHAR_NAME, "<Other Symbol-1f338>"
        )
        == 0x1F338
    )
    with pytest.raises(ICUError) as exc_info:
        _ = u_char_from_name(
            UCharNameChoice.U_EXTENDED_CHAR_NAME, "<So-1f338>"
        )
    ...
    assert (
        u_char_name(0x1F338, UCharNameChoice.U_UNICODE_CHAR_NAME)
        == "CHERRY BLOSSOM"
    )
    ...
    assert (
        u_get_int_property_value(ord("1"), UProperty.UCHAR_EAST_ASIAN_WIDTH)
        == UEastAsianWidth.U_EA_NARROW
    )
    assert (
        u_get_int_property_value(0xFF11, UProperty.UCHAR_EAST_ASIAN_WIDTH)
    ...
    assert (
        u_get_property_name(
            ...
            UPropertyNameChoice.U_LONG_PROPERTY_NAME,
        )
        == "General_Category_Mask"
    )

    assert (
        u_get_property_value_enum(UProperty.UCHAR_DECOMPOSITION_TYPE, "Can")
...
def test_u_get_int_property_map():
    # fmt: off
    from icupy.icu import (
        UCPMapRangeOption, UCPMapValueFilterPtr, u_get_int_property_map,
        ucpmap_get, ucpmap_get_range,
    )
