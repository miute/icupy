...
void init_selfmt(py::module &m) {
  ...
  sf.def(py::init([](const icupy::UnicodeStringVariant &pattern) {
           ...
           auto result = std::make_unique<SelectFormat>(
  ...
  sf.def(
      "___deepcopy__",
      [](const SelectFormat &self, py::dict & /* memo */) {
        return self.clone();
      },
      py::arg("memo"));
  ...
  sf.def(
        ...
        // [2] icu::Format::format
        "format",
        [](const SelectFormat &self, const Formattable &obj,
      ...
      .def(
          ...
          "format",
          [](const SelectFormat &self,
             const icupy::UnicodeStringVariant &keyword,
             UnicodeString &append_to, FieldPosition &pos) -> UnicodeString & {
            ErrorCode error_code;
            auto &result = self.format(icupy::to_unistr(keyword), append_to,
                                       pos, error_code);
            if (error_code.isFailure()) {
              throw icupy::ICUError(error_code);
            }
