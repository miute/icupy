...
void init_unumberrangeformatter(py::module &m) {
  ...
  py::enum_<UNumberRangeCollapse>(
      ...
      .value("UNUM_RANGE_COLLAPSE_NONE", UNUM_RANGE_COLLAPSE_NONE,
             ...
             "Example: \"3.2 thousand kilograms \xe2\x80\x93 5.3 thousand "
             "kilograms\"")
  ...
  py::enum_<UNumberRangeIdentityFallback>(
      ...
      "Defines the behavior when the two numbers in the range are identical "
      "after rounding.\n\n"
      "To programmatically detect when the identity fallback is used, compare "
      "the lower and upper BigDecimals via "
      "*FormattedNumber*.")
      ...
      .value("UNUM_IDENTITY_FALLBACK_APPROXIMATELY_OR_SINGLE_VALUE",
             ...
             "Show the number using a locale-sensitive approximation "
             "pattern.\n\n  "
      ...
      .value("UNUM_IDENTITY_FALLBACK_RANGE", UNUM_IDENTITY_FALLBACK_RANGE,
             ...
             "Use the range pattern always, even if the inputs are the same. "
             "Example (with RangeCollapse.NONE): \"$5 "
             "\xe2\x80\x93 $5\"")
      .export_values();
  ...
  py::enum_<UNumberRangeIdentityResult>(
      ...
      "Used in the result class *FormattedNumberRange* to indicate to the user "
      "whether the numbers formatted in the "
      "range were equal or not, and whether or not the identity fallback was "
      "applied.")
      .value("UNUM_IDENTITY_RESULT_EQUAL_BEFORE_ROUNDING",
             UNUM_IDENTITY_RESULT_EQUAL_BEFORE_ROUNDING,
             "Used to indicate that the two numbers in the range were equal, "
             "even before any rounding rules were "
             "applied.")
      .value("UNUM_IDENTITY_RESULT_EQUAL_AFTER_ROUNDING",
             UNUM_IDENTITY_RESULT_EQUAL_AFTER_ROUNDING,
             "Used to indicate that the two numbers in the range were equal, "
             "but only after rounding rules were "
             "applied.")
      .value("UNUM_IDENTITY_RESULT_NOT_EQUAL", UNUM_IDENTITY_RESULT_NOT_EQUAL,
             ...
             "equal, even after rounding rules were "
             "applied.")
      .export_values();
#endif // (U_ICU_VERSION_MAJOR_NUM >= 63)
}
