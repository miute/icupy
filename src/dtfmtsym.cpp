...
void init_dtfmtsym(py::module &m) {
  ...
  dfs.def(py::init([]() {
      ...
      .def(py::init([](const icupy::LocaleVariant &locale) {
             ErrorCode error_code;
             auto result = std::make_unique<DateFormatSymbols>(
  ...
  dfs.def(
      "__eq__",
      [](const DateFormatSymbols &self, const DateFormatSymbols &other) {
        return self == other;
      },
  ...
  dfs.def(
      ...
      .def(
          ...
          py::return_value_policy::reference, py::arg("context"),
          py::arg("width"))
  ...
  dfs.def(
         "set_am_pm_strings",
         [](DateFormatSymbols &self, const std::list<UnicodeString> &ampms,
            int32_t count) {
           // TODO: Replace std::list with std::vector in the future.
           if (count == -1) {
             count = static_cast<int32_t>(ampms.size());
           }
           std::vector<UnicodeString> values(ampms.begin(), ampms.end());
           self.setAmPmStrings(values.data(), count);
         },
      ...
      .def(
          "set_am_pm_strings",
          [](DateFormatSymbols &self, const std::list<UnicodeString> &ampms,
             int32_t count, DateFormatSymbols::DtContextType context,
             DateFormatSymbols::DtWidthType width) {
            // TODO: Replace std::list with std::vector in the future.
            if (count == -1) {
              count = static_cast<int32_t>(ampms.size());
            ...
            std::vector<UnicodeString> values(ampms.begin(), ampms.end());
            self.setAmPmStrings(values.data(), count, context, width);
          },
          py::arg("ampms"), py::arg("count"), py::arg("context"),
          py::arg("width"))
#endif // (U_ICU_VERSION_MAJOR_NUM >= 78)
      ;
  ...
  dfs.def(
      "set_era_names",
      [](DateFormatSymbols &self, const std::list<UnicodeString> &era_names,
         int32_t count) {
        // TODO: Replace std::list with std::vector in the future.
        if (count == -1) {
          count = static_cast<int32_t>(era_names.size());
        }
        std::vector<UnicodeString> values(era_names.begin(), era_names.end());
        self.setEraNames(values.data(), count);
      },
      py::arg("era_names"), py::arg("count") = -1);

  dfs.def(
      "set_eras",
      [](DateFormatSymbols &self, const std::list<UnicodeString> &eras,
         int32_t count) {
        // TODO: Replace std::list with std::vector in the future.
        if (count == -1) {
          count = static_cast<int32_t>(eras.size());
        }
        std::vector<UnicodeString> values(eras.begin(), eras.end());
        self.setEras(values.data(), count);
      },
      py::arg("eras"), py::arg("count") = -1);

  dfs.def(
      "set_local_pattern_chars",
      [](DateFormatSymbols &self,
         const icupy::UnicodeStringVariant &new_local_pattern_chars) {
  ...
  dfs.def(
         "set_months",
         [](DateFormatSymbols &self, const std::list<UnicodeString> &months,
            int32_t count) {
           // TODO: Replace std::list with std::vector in the future.
           if (count == -1) {
             count = static_cast<int32_t>(months.size());
           }
           std::vector<UnicodeString> values(months.begin(), months.end());
           self.setMonths(values.data(), count);
         },
      ...
      .def(
          ...
          [](DateFormatSymbols &self, const std::list<UnicodeString> &months,
            ...
            self.setMonths(values.data(), count, context, width);
          },
          py::arg("months"), py::arg("count"), py::arg("context"),
          py::arg("width"));

  dfs.def(
      "set_narrow_eras",
      [](DateFormatSymbols &self, const std::list<UnicodeString> &narrow_eras,
         int32_t count) {
        // TODO: Replace std::list with std::vector in the future.
        if (count == -1) {
          count = static_cast<int32_t>(narrow_eras.size());
        }
        std::vector<UnicodeString> values(narrow_eras.begin(),
                                          narrow_eras.end());
        self.setNarrowEras(values.data(), count);
      },
      py::arg("narrow_eras"), py::arg("count") = -1);

  dfs.def(
      "set_quarters",
      [](DateFormatSymbols &self, const std::list<UnicodeString> &quarters,
         int32_t count, DateFormatSymbols::DtContextType context,
         DateFormatSymbols::DtWidthType width) {
        // TODO: Replace std::list with std::vector in the future.
        std::vector<UnicodeString> values(quarters.begin(), quarters.end());
        if (count == -1) {
          count = static_cast<int32_t>(quarters.size());
        }
        self.setQuarters(values.data(), count, context, width);
      },
      py::arg("quarters"), py::arg("count"), py::arg("context"),
      py::arg("width"));

  dfs.def(
      "set_short_months",
      [](DateFormatSymbols &self, const std::list<UnicodeString> &short_months,
         int32_t count) {
        // TODO: Replace std::list with std::vector in the future.
        if (count == -1) {
          count = static_cast<int32_t>(short_months.size());
        }
        std::vector<UnicodeString> values(short_months.begin(),
                                          short_months.end());
        self.setShortMonths(values.data(), count);
      },
  ...
  dfs.def(
      "set_short_weekdays",
      [](DateFormatSymbols &self,
         const std::list<UnicodeString> &abbrev_weekdays, int32_t count) {
        // TODO: Replace std::list with std::vector in the future.
        if (count == -1) {
          count = static_cast<int32_t>(abbrev_weekdays.size());
        }
        std::vector<UnicodeString> values(abbrev_weekdays.begin(),
                                          abbrev_weekdays.end());
        self.setShortWeekdays(values.data(), count);
      },
      py::arg("abbrev_weekdays"), py::arg("count") = -1);

  dfs.def(
         "set_weekdays",
         [](DateFormatSymbols &self, const std::list<UnicodeString> &weekdays,
            int32_t count) {
           // TODO: Replace std::list with std::vector in the future.
           if (count == -1) {
             count = static_cast<int32_t>(weekdays.size());
           }
           std::vector<UnicodeString> values(weekdays.begin(), weekdays.end());
           self.setWeekdays(values.data(), count);
         },
      ...
      .def(
          ...
          [](DateFormatSymbols &self, const std::list<UnicodeString> &weekdays,
            ...
            self.setWeekdays(values.data(), count, context, width);
          },
          py::arg("weekdays"), py::arg("count"), py::arg("context"),
          py::arg("width"));

#if (U_ICU_VERSION_MAJOR_NUM >= 54)
  dfs.def(
      "set_year_names",
      [](DateFormatSymbols &self, const std::list<UnicodeString> &year_names,
         int32_t count, DateFormatSymbols::DtContextType context,
         DateFormatSymbols::DtWidthType width) {
        // TODO: Replace std::list with std::vector in the future.
        std::vector<UnicodeString> values(year_names.begin(), year_names.end());
        if (count == -1) {
          count = static_cast<int32_t>(year_names.size());
        }
        self.setYearNames(values.data(), count, context, width);
      },
      py::arg("year_names"), py::arg("count"), py::arg("context"),
      py::arg("width"));

  dfs.def(
      "set_zodiac_names",
      [](DateFormatSymbols &self, const std::list<UnicodeString> &zodiac_names,
         int32_t count, DateFormatSymbols::DtContextType context,
         DateFormatSymbols::DtWidthType width) {
        // TODO: Replace std::list with std::vector in the future.
        std::vector<UnicodeString> values(zodiac_names.begin(),
                                          zodiac_names.end());
        if (count == -1) {
          count = static_cast<int32_t>(zodiac_names.size());
        }
        self.setZodiacNames(values.data(), count, context, width);
      },
      py::arg("zodiac_names"), py::arg("count"), py::arg("context"),
      py::arg("width"));
#endif // (U_ICU_VERSION_MAJOR_NUM >= 54)

  // FIXME: Implement "void DateFormatSymbols::setZoneStrings(
  //  const UnicodeString *const *strings, int32_t rowCount,
  //  int32_t columnCount)".
}
