...
void init_compactdecimalformat(py::module &m) {
  ...
  //
  py::class_<CompactDecimalFormat, DecimalFormat> cdf(m,
  ...
  cdf.def(
      "__deepcopy__",
      [](const CompactDecimalFormat &self, py::dict & /* memo */) {
        return self.clone();
      },
      py::arg("memo"));
  ...
  cdf.def_static(
      ...
      [](const icupy::LocaleVariant &in_locale, UNumberCompactStyle style) {
        ...
        auto result = CompactDecimalFormat::createInstance(
            icupy::to_locale(in_locale), style, error_code);
