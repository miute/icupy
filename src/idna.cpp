...
void init_idna(py::module &m) {
  ...
  info.def("has_errors",
           [](const IDNAInfo &self) -> py::bool_ { return self.hasErrors(); });

  info.def("is_transitional_different", [](const IDNAInfo &self) -> py::bool_ {
  ...
  py::enum_<decltype(UIDNA_DEFAULT)>(idna, "IDNA", py::arithmetic())
      ...
      .value("ALLOW_UNASSIGNED", UIDNA_ALLOW_UNASSIGNED,
             ...
             "*uidna_open_uts46()* or class *IDNA*.")
      .value("USE_STD3_RULES", UIDNA_USE_STD3_RULES,
             "Option to check whether the input conforms to the STD3 ASCII "
             "rules, for example the restriction of "
             "labels to LDH characters (ASCII Letters, Digits and "
             "Hyphen-Minus).\n\n  "
             "For use in static worker and factory methods.")
      ...
      .value("NONTRANSITIONAL_TO_ASCII", UIDNA_NONTRANSITIONAL_TO_ASCII,
             ...
             "only relevant for compatibility of newer "
             "IDNA implementations with IDNA2003.)")
      .value(
          "NONTRANSITIONAL_TO_UNICODE", UIDNA_NONTRANSITIONAL_TO_UNICODE,
