...
void init_rbnf(py::module &m) {
  ...
  rbnf.def(
          ...
          py::init([](const icupy::UnicodeStringVariant &rules,
            ...
            auto result = std::make_unique<RuleBasedNumberFormat>(
                icupy::to_unistr(rules), perror, error_code);
      ...
      .def(
          ...
          py::init([](const icupy::UnicodeStringVariant &rules,
                      const icupy::UnicodeStringVariant &localizations,
                      UParseError &perror) {
            ErrorCode error_code;
            auto result = std::make_unique<RuleBasedNumberFormat>(
                icupy::to_unistr(rules), icupy::to_unistr(localizations),
                perror, error_code);
            if (error_code.isFailure()) {
              throw icupy::ICUError(error_code);
            }
      ...
      .def(
          ...
          py::init([](const icupy::UnicodeStringVariant &rules,
            ...
            ErrorCode error_code;
            auto result = std::make_unique<RuleBasedNumberFormat>(
                icupy::to_unistr(rules), locale, perror, error_code);
            if (error_code.isFailure()) {
              throw icupy::ICUError(error_code);
            }
            return result;
      ...
      .def(
          ...
          py::init([](const icupy::UnicodeStringVariant &rules,
            ...
            auto result = std::make_unique<RuleBasedNumberFormat>(
                icupy::to_unistr(rules), icupy::to_unistr(localizations),
                icupy::to_locale(locale), perror, error_code);
            if (error_code.isFailure()) {
              throw icupy::ICUError(error_code);
            }
            return result;
          }),
          py::arg("rules"), py::arg("localizations"), py::arg("locale"),
          py::arg("perror"))
      .def(
          ...
          py::init([](URBNFRuleSetTag tag, const icupy::LocaleVariant &locale) {
            ErrorCode error_code;
            auto result = std::make_unique<RuleBasedNumberFormat>(
                tag, icupy::to_locale(locale), error_code);
            if (error_code.isFailure()) {
              throw icupy::ICUError(error_code);
            }
            return result;
  ...
  rbnf.def(
      "__deepcopy__",
      [](const RuleBasedNumberFormat &self, py::dict & /* memo */) {
        return self.clone();
      },
      py::arg("memo"));
  ...
  rbnf.def(
      ...
      .def(
          ...
          "format",
          [](const RuleBasedNumberFormat &self, double number,
             const icupy::UnicodeStringVariant &rule_set_name,
             UnicodeString &to_append_to,
             FieldPosition &pos) -> UnicodeString & {
            ErrorCode error_code;
            auto &result = self.format(number, icupy::to_unistr(rule_set_name),
                                       to_append_to, pos, error_code);
            if (error_code.isFailure()) {
              throw icupy::ICUError(error_code);
            }
            return result;
          },
          py::arg("number").noconvert(), py::arg("rule_set_name"),
          py::arg("to_append_to"), py::arg("pos"))
      .def(
          // [10] icu::NumberFormat::format
          "format",
          py::overload_cast<double, UnicodeString &>(&NumberFormat::format,
                                                     py::const_),
      ...
      .def(
          ...
          "format",
          py::overload_cast<double, UnicodeString &, FieldPosition &>(
              &RuleBasedNumberFormat::format, py::const_),
      ...
      .def(
          ...
          "format",
          [](const RuleBasedNumberFormat &self, int64_t number,
             const icupy::UnicodeStringVariant &rule_set_name,
             UnicodeString &to_append_to,
             FieldPosition &pos) -> UnicodeString & {
            ErrorCode error_code;
          ...
          py::arg("number"), py::arg("rule_set_name"), py::arg("to_append_to"),
          py::arg("pos"))
      .def(
          // [16] icu::NumberFormat::format
          // [22] icu::NumberFormat::format
          "format",
          py::overload_cast<int64_t, UnicodeString &>(&NumberFormat::format,
                                                      py::const_),
      ...
      .def(
          ...
          "format",
          py::overload_cast<int64_t, UnicodeString &, FieldPosition &>(
              &RuleBasedNumberFormat::format, py::const_),
  ...
  rbnf.def("get_default_rule_set_name",
           &RuleBasedNumberFormat::getDefaultRuleSetName);

  rbnf.def("get_number_of_rule_set_display_name_locales",
           &RuleBasedNumberFormat::getNumberOfRuleSetDisplayNameLocales);

  rbnf.def("get_number_of_rule_set_names",
           &RuleBasedNumberFormat::getNumberOfRuleSetNames);
  ...
  rbnf.def("get_rule_set_name", &RuleBasedNumberFormat::getRuleSetName,
           py::arg("index"));

#if (U_ICU_VERSION_MAJOR_NUM >= 49)
  rbnf.def("set_decimal_format_symbols",
           &RuleBasedNumberFormat::setDecimalFormatSymbols, py::arg("symbols"));
