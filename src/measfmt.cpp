...
void init_measfmt(py::module &m) {
  ...
  fmt.def(py::init([](const icupy::LocaleVariant &locale,
            ...
            auto result = std::make_unique<MeasureFormat>(
                icupy::to_locale(locale), width, error_code);
  ...
  fmt.def(
      "__deepcopy__",
      [](const MeasureFormat &self, py::dict & /* memo */) {
        return self.clone();
      },
      py::arg("memo"));
  ...
  fmt.def_static(
         ...
         [](const icupy::LocaleVariant &locale) {
           ErrorCode error_code;
           auto result = MeasureFormat::createCurrencyFormat(
  ...
  fmt.def(
      ...
      [](const MeasureFormat &self, const Measure &measure,
         const MeasureUnit &per_unit, UnicodeString &append_to,
         FieldPosition &pos) -> UnicodeString & {
        ErrorCode error_code;
        auto &result = self.formatMeasurePerUnit(measure, per_unit, append_to,
                                                 pos, error_code);
        if (error_code.isFailure()) {
          throw icupy::ICUError(error_code);
        }
      ...
      py::arg("measure"), py::arg("per_unit"), py::arg("append_to"),
      py::arg("pos"));
  ...
  fmt.def(
      "format_measures",
      [](const MeasureFormat &self, const std::vector<Measure> &measures,
         int32_t measure_count, UnicodeString &append_to,
         FieldPosition &pos) -> UnicodeString & {
        ErrorCode error_code;
        if (measure_count == -1) {
          measure_count = static_cast<int32_t>(measures.size());
        }
        auto &result = self.formatMeasures(measures.data(), measure_count,
                                           append_to, pos, error_code);
        if (error_code.isFailure()) {
          throw icupy::ICUError(error_code);
        }
        return result;
      },
      py::arg("measures"), py::arg("measure_count"), py::arg("append_to"),
      py::arg("pos"));
#endif // (U_ICU_VERSION_MAJOR_NUM >= 53)
