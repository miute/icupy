...
void init_datefmt(py::module &m) {
  ...
  // FIXME: Implement "void icu::DateFormat::adoptCalendar(
  //  Calendar *calendarToAdopt)".

  // FIXME: Implement "void icu::DateFormat::adoptNumberFormat(
  //  NumberFormat *formatToAdopt)".

  // FIXME: Implement "void icu::DateFormat::adoptTimeZone(
  ...
  // FIXME: Implement "static DateFormat
  //  *icu::DateFormat::createInstanceForSkeleton(Calendar *calendarToAdopt,
  ...
  df.def_static(
        ...
        [](const icupy::UnicodeStringVariant &skeleton,
          ...
          auto result = DateFormat::createInstanceForSkeleton(
              icupy::to_unistr(skeleton), icupy::to_locale(locale), error_code);
  ...
  df.def(
        ...
        "format",
        py::overload_cast<Calendar &, UnicodeString &, FieldPosition &>(
            &DateFormat::format, py::const_),
      ...
      .def(
          ...
          [](const DateFormat &self, Calendar &cal, UnicodeString &append_to,
             std::optional<FieldPositionIterator *> &pos_iter)
              -> UnicodeString & {
            ErrorCode error_code;
            auto &result = self.format(cal, append_to,
      ...
      .def(
          ...
          // [1] icu::Format::format
          "format",
          [](const DateFormat &self, const Formattable &obj,
      ...
      .def(
          ...
          "format",
          py::overload_cast<UDate, UnicodeString &>(&DateFormat::format,
                                                    py::const_),
          py::arg("date"), py::arg("append_to"))
      .def(
          // [9] icu::DateFormat::format
          "format",
          py::overload_cast<UDate, UnicodeString &, FieldPosition &>(
              &DateFormat::format, py::const_),
      ...
      .def(
          ...
          [](const DateFormat &self, UDate date, UnicodeString &append_to,
             std::optional<FieldPositionIterator *> &pos_iter)
              -> UnicodeString & {
            ErrorCode error_code;
            auto &result = self.format(date, append_to,
                                       pos_iter.value_or(nullptr), error_code);
            if (error_code.isFailure()) {
              throw icupy::ICUError(error_code);
            }
  ...
  df.def(
      ...
      [](const DateFormat &self,
         UDateFormatBooleanAttribute attr) -> py::bool_ {
        ErrorCode error_code;
  ...
  df.def("get_calendar", &DateFormat::getCalendar,
         py::return_value_policy::reference);
...
#if (U_ICU_VERSION_MAJOR_NUM >= 53)
  df.def("is_calendar_lenient", [](const DateFormat &self) -> py::bool_ {
    return self.isCalendarLenient();
  });
#endif // (U_ICU_VERSION_MAJOR_NUM >= 53)

  df.def("is_lenient",
         [](const DateFormat &self) -> py::bool_ { return self.isLenient(); });

  df.def(
        // [1] icu::DateFormat::parse
        "parse",
        [](const DateFormat &self, const icupy::UnicodeStringVariant &text,
           Calendar &cal, ParsePosition &pos) {
  ...
  df.def(
        "parse_object",
        [](const DateFormat &self, const icupy::UnicodeStringVariant &source,
  ...
  df.def(
      ...
      [](DateFormat &self, UDateFormatBooleanAttribute attr,
         py::bool_ new_value) -> DateFormat & {
        ErrorCode error_code;
...
#if (U_ICU_VERSION_MAJOR_NUM >= 53)
  df.def(
      "set_calendar_lenient",
      [](DateFormat &self, py::bool_ lenient) {
        self.setCalendarLenient(lenient);
      },
      py::arg("lenient"));
  ...
  df.def(
      "set_lenient",
      [](DateFormat &self, py::bool_ lenient) { self.setLenient(lenient); },
      py::arg("lenient"));

  df.def("set_number_format", &DateFormat::setNumberFormat,
         py::arg("new_number_format"));
